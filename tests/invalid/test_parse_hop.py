...
def test_parse_invalid_hop_form():
    with open(os.path.join(os.path.dirname(__file__), "../assets/invalid/hop/invalid_form.xml")) as file_content:
        xml_content = file_content.read()
    try:
        BeerxmlParser().parse(xml_content)
    except Exception as err:
        assert 1 == err.error_count()
        assert ("HOPS", 0, "FORM") == err.errors()[0]["loc"]
...
def test_parse_invalid_hop_type():
    with open(os.path.join(os.path.dirname(__file__), "../assets/invalid/hop/invalid_type.xml")) as file_content:
        xml_content = file_content.read()
    try:
        BeerxmlParser().parse(xml_content)
    except Exception as err:
        assert 1 == err.error_count()
        assert ("HOPS", 0, "TYPE") == err.errors()[0]["loc"]
